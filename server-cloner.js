...
const cloneServer = async (client, sourceGuildId, newServerName, socket) => {
    try {
        // 1. Kaynak ve hedef sunucuları al
        ...
        const newGuild = await client.guilds.create(newServerName, {
        ...
        for (const sourceChannel of sourceChannels) {
            const channelOptions = {
                ...
                permissionOverwrites: sourceChannel.permissionOverwrites.cache.map(overwrite => ({
                    id: roleMap.get(overwrite.id)?.id,
                    allow: overwrite.allow.toArray(),
                    deny: overwrite.deny.toArray()
                })).filter(ow => ow.id) // Eşleşmeyen rolleri filtrele
            };
            ...
            try {
                ...
                if (sourceChannel.type === 'GUILD_CATEGORY') {
